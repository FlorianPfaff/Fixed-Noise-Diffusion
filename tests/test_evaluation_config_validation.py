...
class _IdentityDiffusion:
    ...
    def sample(self, *args, **kwargs):
        raise AssertionError("sample should not be called for invalid sample_steps")
...
def test_denoising_loss_rejects_nonpositive_batch_budget(batches):
    with pytest.raises(ValueError, match="evaluation.denoising_batches"):
        denoising_loss_from_timesteps(
            ...
            loader=_loader(),
            sampler=type("Sampler", (), {"sample": lambda self, n: torch.zeros(n, 1, 2, 2)})(),
...
def test_sample_grid_rejects_negative_sample_count(tmp_path):
    config = {
        "evaluation": {"sample_count": -1, "sample_steps": 1, "sampler": "ddim"},
...
def test_sample_grid_rejects_nonpositive_sample_steps(tmp_path):
    config = {
        "evaluation": {"sample_count": 1, "sample_steps": 0, "sampler": "ddim"},
        "data": {"channels": 1, "image_size": 2},
    }

    with pytest.raises(ValueError, match="evaluation.sample_steps"):
        sample_grid(
            _ZeroModel(),
            _IdentityDiffusion(),
            config,
